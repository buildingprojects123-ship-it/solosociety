...
export default function BottomNav() {
  ...
  const navItems = [
    ...
    { href: '/dinners', label: 'Dinners', icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-sm border-t border-gray-200 z-20 shadow-lg">
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <span className="absolute top-0 left-1/2 transform -translate-x-1/2 w-12 h-1 bg-primary-600 rounded-b-full" />
                )}
              </Link>
            )
          })}
